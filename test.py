...
class MacroApp:
    ...
    def build_casting_tab(self, parent):
        ...
        devil_fruit_hotkey_combobox = ttk.Combobox(
            devil_fruit_hotkey_frame,
            textvariable=self.devil_fruit_hotkey_var,
        ...
        rod_hotkey_combobox = ttk.Combobox(
            rod_hotkey_frame,
            textvariable=self.rod_hotkey_var,
            values=list(range(10)),
            width=5,
            font=("Arial", 10),
            state="readonly"
        ...
        anything_else_hotkey_combobox = ttk.Combobox(
            store_fruit_hotkey_frame,
            textvariable=self.anything_else_hotkey_var,
            values=list(range(10)),
            width=5,
            font=("Arial", 10),
            state="readonly"
    ...
    def detect_devil_fruit_in_drop(self):
        ...
        try:
            ...
            detected_text = pytesseract.image_to_string(pil_image).lower()
        ...
        # Require at least 70% of words
        message1_threshold = int(len(message1_words) * 0.7)
        message2_threshold = int(len(message2_words) * 0.7)
    ...
    def check_and_store_devil_fruit(self):
        ...
        if not self.drop_area:
            print("[DEBUG] check_and_store_devil_fruit(): No drop area configured, skipping fruit detection")
            # Fall back to always store if no drop area
            print("[DEBUG] check_and_store_devil_fruit(): Calling run_auto_store_devil_fruit()")
    ...
    def run_auto_store_devil_fruit(self):
        ...
        try:
            ...
            # 3. Click on store_df_area (click at center of area)
            if self.store_df_area:
                # Calculate center of the area
                x1 = self.store_df_area["x1"]
                y1 = self.store_df_area["y1"]
                x2 = self.store_df_area["x2"]
                y2 = self.store_df_area["y2"]
                center_x = (x1 + x2) // 2
                center_y = (y1 + y2) // 2
                click_point = (center_x, center_y)
                print(f"[DEBUG] run_auto_store_devil_fruit(): Clicking on store_df_area center: {click_point}")
            elif self.store_fruit_point:
                click_point = self.store_fruit_point
                print(f"[DEBUG] run_auto_store_devil_fruit(): Clicking on store_fruit_point: {click_point}")
            else:
                print(f"[DEBUG] run_auto_store_devil_fruit(): No store_df_area or store_fruit_point set, returning")
                return
    ...
    # Fishing stages
    def cast(self):
        print(f"[DEBUG] cast() called, fishing_state={self.fishing_state}")
        if self.fishing_state == "IDLE":
            print("[DEBUG] cast(): In IDLE state, about to double click to cast")
            ...
            if self.water_point:
                print(f"[DEBUG] cast(): Moving to water point: {self.water_point}")
                self.mouse_controller.position = self.water_point
            else:
                print("[DEBUG] cast(): No water point set, using current position")
            
            current_pos = self.mouse_controller.position
            print(f"[DEBUG] cast(): Current mouse position: {current_pos}")
            print("[DEBUG] cast(): Performing double click to cast")
            self.mouse_controller.click(Button.left, 2)  # Double click
            self.fishing_state = "CASTING"
            self.cast_time = time.time()
            print(f"[DEBUG] cast(): State changed to CASTING, cast_time={self.cast_time}")
            ...
            print(f"[DEBUG] cast(): In CASTING state, time since cast: {time_since_cast:.2f}s")
            if time_since_cast > 1.0:  # Wait 1 second after cast
                print("[DEBUG] cast(): 1 second elapsed, switching to WAITING state")
                self.fishing_state = "WAITING"
                self.waiting_start_time = time.time()  # Mark when we started waiting
                print(f"[DEBUG] cast(): waiting_start_time={self.waiting_start_time}")
            else:
                print(f"[DEBUG] cast(): Still waiting for 1 second to elapse...")
    
    def waiting(self):
        print(f"[DEBUG] waiting() called, fishing_state={self.fishing_state}, waiting_start_time={self.waiting_start_time}")
        ...
        if self.waiting_start_time is not None:
            ...
            print(f"[DEBUG] waiting(): Time spent waiting so far: {time_waiting:.2f}s")
            if time_waiting < 0.3:
                print("[DEBUG] waiting(): Less than 0.3s, still settling, returning")
                return  # Don't check yet, screen might still have residual blue
            else:
                print("[DEBUG] waiting(): Screen should be settled, checking for blue...")
        else:
            print("[DEBUG] waiting(): waiting_start_time is None, setting it now")
            self.waiting_start_time = time.time()
            return
        
        # Check if blue color appears
        blue_detected = self.check_for_blue()
        print(f"[DEBUG] waiting(): blue_detected={blue_detected}")
        
        if blue_detected:
            print("[DEBUG] waiting(): Blue detected! Switching to FISHING state")
            self.fishing_state = "FISHING"
            self.blue_lost_time = None
            self.waiting_start_time = None
        else:
            print("[DEBUG] waiting(): Blue NOT detected")
            # Check if we've been waiting too long (15 seconds max from when we started waiting)
            if self.waiting_start_time is not None:
                time_in_waiting = time.time() - self.waiting_start_time
                print(f"[DEBUG] waiting(): Total time in waiting: {time_in_waiting:.2f}s")
                if time_in_waiting > 15.0:
                    print("[DEBUG] waiting(): Timeout (15s), switching back to IDLE to recast")
                    self.fishing_state = "IDLE"
                    self.cast_time = None
                    self.waiting_start_time = None
                else:
                    print(f"[DEBUG] waiting(): Still waiting for blue to appear... ({time_in_waiting:.2f}s / 15s)")
            ...
            if self.recast_locked:
                ...
                if remaining > 0:
                    ...
                    if self.interruptible_sleep(remaining):
                        ...
                        # Check if stop was requested and return early
                        if self.stop_requested:
                            print("[DEBUG] fishing(): Stop requested during recast_locked wait, returning")
                            return
                    else:
